...
public class PaymentsDAO implements DAOInterface, IStructureCheckable {
    ...
    public boolean InsertPayment(Payments payment) throws SQLException {
        ...
        try {
            ...
            pStmt.setInt(1, payment.getDetaillines());
            pStmt.setBigDecimal(2, payment.getAmount());
            pStmt.setTimestamp(3, Convert.ToSQLDateTime(payment.getEntered()));
            //pStmt.setDate(3, Convert.ToSQLDate(payment.getEntered()));
            SQLUtil.SafeSetString(pStmt, 4, payment.getUser());
            pStmt.executeUpdate();
    ...
    public boolean UpdatePayment(Payments payment) throws SQLException {
        ...
        try {
            String stmt = "UPDATE " + table + " SET"
                    ...
                    + " `entered` = ?,"
                    + " `user` = ? "
                    + "WHERE `idtransactions` = " + payment.getIdtransactions();
            ...
            pStmt.setBigDecimal(2, payment.getAmount());
            pStmt.setTimestamp(3, Convert.ToSQLDateTime(payment.getEntered()));
            pStmt.setString(4, payment.getUser());
    ...
    public boolean AddUser(String user, int id) throws SQLException {
        ...
        try {
            ...
            System.out.println("Unable to update Payment user: " + user + " id: " + id + " MSG: "
                    + ex.toString());
            return false;
        }
    }

    public Payments GetPaymentByTransaction(int ID) throws SQLException {
        try {
            if (con.isClosed()) {
                con = CheckDBConnection.Check(dbs, con);
            }
        ...
        try {
            ...
            if (rs.next()) {
                ...
                payment.setIdtransactions(rs.getInt("idtransactions"));
                payment.setDetaillines(rs.getInt("detail"));
                payment.setAmount(rs.getBigDecimal("amount"));
                payment.setEntered(rs.getTimestamp("entered"));
                payment.setUser(rs.getString("user"));
            }
    ...
    public int GetIdFromJournalExport(BigDecimal payment, java.util.Date entered, int arNo, BigDecimal amtBilled) throws SQLException {
        ...
        try {
            ...
            System.out.println("Failed finding payment transaction id. Message: " + ex.toString());
            return 0;
        }
    }
    ...
    public String structureCheck() {
        String query = "SELECT `payments`.`idtransactions`,\n"
                + "    `payments`.`detail`,\n"
                + "    `payments`.`amount`,\n"
                + "    `payments`.`entered`,\n"
                + "    `payments`.`user`\n"
                + "FROM `css`.`payments` LIMIT 1";
