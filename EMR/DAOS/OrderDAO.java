...
public class OrderDAO implements IOrderDAO, DAOInterface, IStructureCheckable {
    ...
    public Orders GetOrderNonApproved(String AccessionNumber) throws SQLException {
        try {
            if (con.isClosed()) {
                con = CheckDBConnection.Check(dbs, con);
            }
        ...
        try {
            ...
            String query = "SELECT * FROM " + table
                    + "LEFT JOIN emrorders.emrApprovalLog emrApp ON " + table + ".idOrders = emrApp.idOrders "
                    + "WHERE `accession` LIKE ? "
                    + "AND emrApp.id IS NULL";

            String accessionParam = SQLUtil.createSearchParam2Way(AccessionNumber);
            stmt = createStatement(con, query, accessionParam);
            ResultSet rs = stmt.executeQuery();
    ...
    public List<Orders> GetAllOrdersWithAccessionNonApproved(String AccessionNumber) throws SQLException {
        try {
            if (con.isClosed()) {
                con = CheckDBConnection.Check(dbs, con);
            }
    ...
    public int AccessionCountNonApprove(String acc) {
        try {
            if (con.isClosed()) {
                con = CheckDBConnection.Check(dbs, con);
            }
        ...
        try {
            ...
            rs = stmt.executeQuery("SELECT COUNT(*) FROM " + table 
                    + " LEFT JOIN emrorders.emrApprovalLog emrApp ON " + table + ".idOrders = emrApp.idOrders"
                    + " WHERE "
                    + "`accession` LIKE \"%" + acc + "%\" AND emrApp.id IS NULL");
            rs.next();
            rowCount = rs.getInt(1);
    ...
    public ArrayList<Integer> GetOrderIDsByDateRange(String StartDate, String EndDate, String DateRange) throws SQLException
    {
        try
        {
            if (con.isClosed())
        ...
        try
        {
            ...
            String query = "SELECT o.`idOrders` FROM "
                    + table + " o "
                    + "LEFT JOIN emrorders.emrApprovalLog e "
                    + "ON o.idOrders = e.idOrders "
    ...
    public List<Orders> SearchOrdersByPatientName(String FirstNameFragment, String LastNameFragment) {
        ...
        try {
            ...
            String query = "SELECT * FROM css.orders o "
                    + "LEFT OUTER JOIN css.patients p "
